    ...
    def run(self):
        ...
        for i in range(len(self.data) // 8 * self.id, min(len(self.data) // 8 * (self.id + 1), len(self.data))):
            ...
            if pattern.findall(self.data[i]):
                ...
                if self.data[i] in self.corpus.dictionary.idx2word:
...
class Corpus(object):
    ...
    def generate_train_set(self, path):
        ...
        self.unique = dict(sorted(self.unique.items(), key=operator.itemgetter(1), reverse = True))
        ...
        for k, v in self.unique.items():
            pattern = re.compile(r'●')
            if not pattern.findall(k):
                self.dictionary.add_word(k)
        
            if len(self.dictionary) >= 50000:
                break
    ...
    def generate_test_set(self, path):
        ...
        with open('word_data/actual_test_data', 'wb') as handle:
            pickle.dump(test_data_list, handle)
